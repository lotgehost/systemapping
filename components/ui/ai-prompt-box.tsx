  ...
  return (
    <>
      <PromptInput
        ...
        className={cn('w-full', isRecording && 'border-red-500/70', className)}
      ...
      >
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div className={cn('flex items-center gap-1 transition-opacity duration-300', isRecording ? 'opacity-0 invisible h-0' : 'opacity-100 visible')}>
            ...
            <div className="flex items-center">
              <button type="button" onClick={() => handleToggleChange('search')} className={cn('rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8', showSearch ? 'bg-[#1EAEDB]/15 border-[#1EAEDB] text-[#1EAEDB]' : 'bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]')}>
                ...
                <AnimatePresence>
                  {showSearch && <motion.span initial={{ width: 0, opacity: 0 }} animate={{ width: 'auto', opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="text-xs overflow-hidden whitespace-nowrap flex-shrink-0">Search</motion.span>}
                </AnimatePresence>
              </button>
              ...
              <button type="button" onClick={() => handleToggleChange('think')} className={cn('rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8', showThink ? 'bg-[#8B5CF6]/15 border-[#8B5CF6] text-[#8B5CF6]' : 'bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]')}>
                ...
                <AnimatePresence>
                  {showThink && <motion.span initial={{ width: 0, opacity: 0 }} animate={{ width: 'auto', opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="text-xs overflow-hidden whitespace-nowrap flex-shrink-0">Think</motion.span>}
                </AnimatePresence>
              </button>
              ...
              <button type="button" onClick={() => setShowCanvas((p) => !p)} className={cn('rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8', showCanvas ? 'bg-[#F97316]/15 border-[#F97316] text-[#F97316]' : 'bg-transparent border-transparent text-[#9CA3AF] hover:text-[#D1D5DB]')}>
                ...
                <AnimatePresence>
                  {showCanvas && <motion.span initial={{ width: 0, opacity: 0 }} animate={{ width: 'auto', opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="text-xs overflow-hidden whitespace-nowrap flex-shrink-0">Canvas</motion.span>}
                </AnimatePresence>
              </button>
            </div>
          </div>
          ...
          <PromptInputAction tooltip={isLoading ? 'Stop generation' : hasContent ? 'Send message' : 'Voice message'}>
            ...
            >
              {isLoading ? <Square className="h-4 w-4 fill-[#1F2023] animate-pulse" /> : hasContent ? <ArrowUp className="h-4 w-4 text-[#1F2023]" /> : <Mic className="h-5 w-5" />}
            </Button>
          </PromptInputAction>
        </PromptInputActions>
      </PromptInput>
